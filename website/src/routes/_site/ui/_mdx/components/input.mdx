...
<ComponentUsage
  preview={(props) => <Input placeholder={"输入框"} {...props} />} componentFile={"./input/Input.tsx"}
  ignoreProps={["value", "defaultValue", "placeholder", "htmlSize", "leading", "leadingWidth", "leadingPointer", "trailing", "trailingWidth", "trailingPointer"]} />
...
<section className={'example group'}>
  <ComponentExample className={"flex"}>
    {() => {
      ...
      return (
        <div className={"flex flex-col gap-2"}>
          <Input clearable placeholder="输入框"
                 onChange={(v) => setValue(prev => [...prev, `onChange: "${v}"`])}
                 onClear={(v) => setValue(prev => [...prev, `onClear`])}/>
          <div className={"flex gap-2"}>
            <span>事件: </span>
            <pre className={"text-xs p-1"} data-inline={true}>{value.join('\n')}</pre>
          </div>
        </div>
      );
    }}
...
<section className={'example group'}>
  <ComponentExample className={"flex"}>
    <div className={"flex flex-col gap-3"}>
      ...
      <Input leading={<Button variant={"ghost"} color={"neutral"} size={"xs"} square={true}><SpriteIcon
        size={"sm"} name={"search"} /></Button>} leadingPointer={true} placeholder={"搜索..."} />
      <Input clearable placeholder={"邮件地址"}
             trailing={<Button size={"xs"}>订阅</Button>} trailingWidth={56} trailingPointer={true} />
    </div>
  </ComponentExample>
...
<ComponentProps componentFile={"./input/InputGroup.tsx"} />
