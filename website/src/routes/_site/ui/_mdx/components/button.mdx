...
import { ComponentProps, ComponentUsage } from "../__components";
...
<ComponentUsage preview={(props) => <Button {...props}>按钮</Button>} componentFile={"./button/Button.tsx"} />

## 外观

按钮有五种不同的外观可供使用。通过传递 `variant` 属性，可以选择使用 `solid`、`outline`、`soft`、`ghost` 或 `link` 中的任意一种。
...
按钮圆角可以设置为 `false`、`xs`、`sm`、`lg`、`xl`、`full` 以及数字
...
<section className={'demo group'}>
  <div
    ...
    <Button square variant={'soft'} color={'warning'}>
      <SpriteIcon size={'sm'} name={'logout'} />
    </Button>
    <Button square variant={'ghost'} color={'success'}>
...
### 按钮
<ComponentProps componentFile={"./button/Button.tsx"} />

### 按钮组
<ComponentProps componentFile={"./button/ButtonGroup.tsx"} />
